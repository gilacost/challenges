...
///
///"""
/// use challenges::matrix::correct_path::make;
/// make("???rrurdr?") == "dddrrurdrd;
///
/// use challenges::matrix::correct_path::make;
/// make("drdr??rrddd?") == "drdruurrdddd";
///"""
///
...
pub fn make(input_string: String) -> String {
...
#[cfg(test)]
mod tests {
    use crate::matrix::correct_path::make;

    #[test]
    fn path_correction() {
        let path_to_correct = "???rrurdr?".to_string();
        let expected = "dddrrurdrd".to_string();
        assert_eq!(make(path_to_correct), expected);
